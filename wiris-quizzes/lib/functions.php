...
function wrsqz_checkBool($value) {
    $truesarray = array('true','cierto','vrai','vero','waar','tõene','cert','ziur');
 ...
 *
 * The implementation of this function is a bit confusing, so there are tests for
 * this function in /lib/unittests.php
 *
 * **/
      ...
      if(in_array($parenttagname, $allowedparenttags)){
        ...
        while(!$end && $nextpos !==FALSE && ($pos + strlen($closetag)) < strlen($formula)){ //not the end of formula
          ...
          if($nexttag == $tag || $nexttag == $opentag.'mn'.$closetag){
            ...
            if($nextclosepos!=$contentpos){ //remains some content that is not part of variable name
...
function wrsqz_embeddedShortanswerInput($wrapped, $value, $inputname, $style = '', $readonly = '', $popup = ''){
    ...
    if(empty($value)){
        ...
        echo "<img src=\"".$CFF->dirroot .'/pluginwiris/editor/wiris-formula.gif'."\"/>";
...
function wrsqz_wirisEditorAppletHTML($prefix, $grammar, $grammarTarget, $wirisFormulaContent = '' ,$width=500, $height=265){
  ...
  $html .= '<param name="grammar.1.target" value="'.$grammarTarget.'"/>'."\n";
	$html .= '</applet>'."\n";
  //hidden fields
  $html .= '<input type="hidden" id="'.$prefix.'wirisEditorResponse" name="'.$prefix.'" value="'.$wirisFormulaContent.'" size="80"/>';
  $html .= '<input type="hidden" id="'.$prefix.'wirisEditorHidden" name="'.$prefix.'wirisEditorHidden"	value="" size="80"/>'."\n";
	$html .= '</div>';
	return $html;
}
