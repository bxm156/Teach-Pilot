...
class block_modAdmin extends block_list {
    function init() {
        $this->title = 'ModAdmin';
        $this->version = 20100910;
    }
...
/////////////////////
    ...
    /// Course editing on/off
        ...
        if ($course->id !== SITEID and has_capability('moodle/course:update', $context)) {
            ...
            if (isediting($this->instance->pageid)) {
                $this->content->items[]='<a href="/course/view.php?id='.$COURSE->id.'&amp;edit=off&amp;sesskey='.sesskey().'">'.get_string('turneditingoff').'</a>';
            } else {
                $this->content->items[]='<a href="/course/view.php?id='.$COURSE->id.'&amp;edit=on&amp;sesskey='.sesskey().'">'.get_string('turneditingon').'</a>';
            }
    ...
    function applicable_formats() {
        return array('all' => false,'mod-*' => true);   // Not needed on site
    }
}
