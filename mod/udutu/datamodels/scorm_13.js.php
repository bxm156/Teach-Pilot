<?php
    if (isset($userdata->status)) {
        ...
        if (!isset($userdata->{'cmi.exit'})) {
		$userdata->entry = 'ab-initio';
	  } else if(($userdata->{'cmi.exit'} == 'time-out') || ($userdata->{'cmi.exit'} == 'normal')) { 
                $userdata->entry = '';
...
function SCORMapi1_3() {
    ...
    var datamodel =  {
	  ...
	  'cmi.comments_from_learner._count':{'mod':'r','defaultvalue':'<?php echo isset($userdata->{'comments_from_leaner_count'})?$userdata->{'comments_from_learner_count'}:'0'?>'},
	  ...
	  'cmi.comments_from_lms._count':{'mod':'r', 'defaultvalue':'<?php echo isset($userdata->{'comments_from_lms_count'})?$userdata->{'comments_from_lms_count'}:'0' ?>'},
	  ...
	  'cmi.interactions._count':{'mod':'r', 'defaultvalue':'<?php echo isset($userdata->{'interactions_count'})?$userdata->{'interactions_count'}:'0' ?>'},
	  ...
	  'cmi.objectives._count':{'mod':'r', 'defaultvalue':'<?php echo isset($userdata->{'objectives_count'})?$userdata->{'objectives_count'}:'0' ?>'},
...
<?php
	...
	$count = $userdata->{'objectives_count'};
	for( $i=0; $i < $count; $i++ ){
		echo '     cmi.objectives.N'.$i." = new Object();\n";
		echo '     cmi.objectives.N'.$i.".score = new Object();\n";
		echo '     cmi.objectives.N'.$i.".score = score_children;\n";
	}
...
<?php
	$count = $userdata->{'comments_from_learner_count'};
	for( $i=0; $i < $count; $i++ ){
		echo '     cmi.comments_from_learner.N'.$i." = new Object();\n";
	}
	
	foreach($userdata as $element => $value){
		if (substr($element,0,25) == 'cmi.comments_from_learner') {
			preg_match('/\.(\d+)\./',$element,$matches);
			$element = preg_replace('/\.(\d+)\./',".N\$1.",$element);
			echo '    '.$element.' = \''.$value."';\n";
		}
	...
	$count = $userdata->{'comments_from_lms_count'};
	for( $i=0; $i < $count; $i++ ){
		echo '     cmi.comments_from_lms.N'.i." = new Object();\n";
	}
	
	foreach($userdata as $element => $value){
		if (substr($element,0,20) == 'cmi.comments_from_lms') {
			preg_match('/\.(\d+)\./',$element,$matches);
			$element = preg_replace('/\.(\d+)\./',".N\$1.",$element);
			echo '    '.$element.' = \''.$value."';\n";
		}
	...
	$count = $userdata->{'interactions_count'};
	for( $i=0; $i < $count; $i++ ){
		echo '     cmi.interactions.N'.$i." = new Object();\n";
		echo '     cmi.interactions.N'.$i.".correct_responses = new Object();\n";
		echo '     cmi.interactions.N'.$i.".objectives = new Object();\n";
	}
	
	$interactionCorrectResponseCountArray = array();
	$interactionObjectivesCountArray = array();
	$paddedInterCorrectResponseCountArray = array_pad($interactionCorrectResponseCountArray, $userdata->{'interactions_count'}, 0 );
	$paddedInterObjectivesCountArray = array_pad($interactionObjectivesCountArray, $userdata->{'interactions_count'}, 0 );
	foreach($userdata as $element => $value){
		if (substr($element,0,16) == 'cmi.interactions') {
			preg_match('/\.(\d+)\./',$element,$matches);
			$element = preg_replace('/\.(\d+)\./',".N\$1.",$element);
			$element_split = split('\.',$element);
			if( ($element_split[3] == 'correct_responses') && ($element_split[5] == 'pattern') ) {
				echo '    '.$element_split[0].'.'.$element_split[1].'.'.$element_split[2].'.'.$element_split[3].'.'.$element_split[4]." = new Object();\n";
				$index = substr($element_split[2],1);
				$paddedInterCorrectResponseCountArray[$index]++;
			}
			if( ($element_split[3] == 'objectives') && ($element_split[5] == 'id') ) {
				echo '    '.$element_split[0].'.'.$element_split[1].'.'.$element_split[2].'.'.$element_split[3].'.'.$element_split[4]." = new Object();\n";
				$index = substr($element_split[2],1);
				$paddedInterObjectivesCountArray[$index]++;
			}
			echo '    '.$element.' = \''.$value."';\n";
		}
	}
	foreach( $paddedInterCorrectResponseCountArray as $i => $value){
			echo '    cmi.interactions.N'.$i.'.correct_responses._count = '.$paddedInterCorrectResponseCountArray[$i].";\n";
	}
	foreach( $paddedInterObjectivesCountArray as $i => $value){
			echo '    cmi.interactions.N'.$i.'.objectives._count = '.$paddedInterObjectivesCountArray[$i].";\n";
	}
...
//                    if (<?php echo $scorm->auto ?> == 1) {
                                                    ...
                                                    if (typeof eval(subelement) == "undefined") {
                                                        ...
                                                        if ((elemlen <= maxmodel.length) && (element.substr(elemlen-4) == 'type') && (errorCode=="0")) { 
                                                           ...
                                                           subobject.learner_response = new Object();
								    														...
								    														elementSplitArray = element.split(".");
														
														if( elementSplitArray[3] == "objectives") {
															parentmodel = 'cmi.interactions.'+ elementSplitArray[2]+'.objectives';
															maxmodel = 'cmi.interactions.Nxxx.objectives.Nxxx.id';
															if ((elemlen <= maxmodel.length) && (element.substr(elemlen-2) == 'id') && (errorCode=="0")) { 
																eval(parentmodel+'._count++;');
																eval(subelement+' = new Object();');
																subobject = eval(subelement);
																subobject.id = datamodel["cmi.interactions.n.objectives.n.id"].defaultvalue; 
															} 
														} else if( elementSplitArray[3] == "correct_responses") {
															parentmodel = 'cmi.interactions.'+ elementSplitArray[2]+'.correct_responses';
															maxmodel = 'cmi.interactions.Nxxx.correct_responses.Nxxx.pattern';
															if ((elemlen <= maxmodel.length) && (element.substr(elemlen-7) == 'pattern') && (errorCode=="0")) { 
																eval(parentmodel+'._count++;');
																eval(subelement+' = new Object();');
																subobject = eval(subelement);
																subobject.pattern = datamodel["cmi.interactions.n.correct_responses.n.pattern"].defaultvalue; 
															} 
														}
    ...
    function StoreData(data,storetotaltime) {
        ...
        result = DoRequest(myRequest,"<?php p($CFG->wwwroot) ?>/mod/udutu/datamodel.php","id=<?php p($id) ?>&sesskey=<?php p($USER->sesskey) ?>"+datastring);
        <?php
            if (debugging('',DEBUG_DEVELOPER)) {
