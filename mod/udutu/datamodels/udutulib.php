<?php
function udutu_get_resources($blocks) {
    $resources = array();
...
function udutu_get_manifest($blocks,$scoes) {
        ...
        foreach ($blocks as $block) {
            switch ($block['name']) {
                ...
                case 'MANIFEST':
                    ...
                    $organization = '';
                    $resources = array();
                    $resources = udutu_get_resources($block['children']);
                    $scoes = udutu_get_manifest($block['children'],$scoes);
                    if (count($scoes->elements) <= 0) {
                        foreach ($resources as $item => $resource) {
                            if (!empty($resource['HREF'])) {
                                ...
                                $sco->udututype = addslashes($resource['ADLCP:SCORMTYPE']);
                ...
                case 'ORGANIZATION':
                    ...
                    $scoes->elements[$manifest][$organization][$identifier]->udututype = '';
                ...
                case 'ITEM':
                    ...
                    if (!isset($block['attrs']['IDENTIFIERREF'])) {
                        $scoes->elements[$manifest][$organization][$identifier]->launch = '';
                        $scoes->elements[$manifest][$organization][$identifier]->udututype = 'asset';
                    } else {
                        ...
                        $scoes->elements[$manifest][$organization][$identifier]->udututype = addslashes($resources[$idref]['ADLCP:SCORMTYPE']);
                    }
...
function udutu_parse_udutu($pkgdir,$udutuid) {
    global $CFG;
...
//print_object($manifests); 
        $scoes = new stdClass();
        $scoes->version = '';
        $scoes = udutu_get_manifest($manifests,$scoes);
//print_object($scoes);
        if (count($scoes->elements) > 0) {
            $olditems = get_records('udutu_scoes','udutu',$udutuid);
            foreach ($scoes->elements as $manifest => $organizations) {
                foreach ($organizations as $organization => $items) {
                    foreach ($items as $identifier => $item) {
                        // This new db mngt will support all udutu future extensions
                        $newitem = new stdClass(); 
                        $newitem->udutu = $udutuid;
                        $newitem->manifest = $manifest;
                        $newitem->organization = $organization;
                        $standarddatas = array('parent', 'identifier', 'launch', 'udututype', 'title');
                        foreach ($standarddatas as $standarddata) {
                            $newitem->$standarddata = addslashes($item->$standarddata);
                        }
                        
                        $id = 0; 
                        if (!empty($olditems) && ($olditemid = udutu_array_search('identifier',$newitem->identifier,$olditems))) {
                            $newitem->id = $olditemid;
                            $id = update_record('udutu_scoes',$newitem);
                            unset($olditems[$olditemid]);
                            delete_records('udutu_scoes_data','scoid',$olditemid);
                        } else {
                            $id = insert_record('udutu_scoes',$newitem);
                        }

                        if ($optionaldatas = udutu_optionals_data($item,$standarddatas)) {
                            $data = new stdClass();
                            $data->scoid = $id;
                            foreach ($optionaldatas as $optionaldata) {
                                if (isset($item->$optionaldata)) {
                                    ...
                                    $dataid = insert_record('udutu_scoes_data',$data);
                                }
                            }
                        }
                        ...
                        if (isset($item->sequencingrules)) {
                            foreach($item->sequencingrules as $sequencingrule) {
                                ...
                                $ruleid = insert_record('udutu_seq_ruleconds',$rule);
                                if (isset($sequencingrule->ruleconditions)) {
                                    foreach($sequencingrule->ruleconditions as $rulecondition) {
                                        ...
                                        $rulecondid = insert_record('udutu_seq_rulecond',$rulecond);
                                    }
                                }
                            }                        
                        }
                        ...
                        if (isset($item->rolluprules)) {
                            foreach($item->rolluprules as $rolluprule) {
                                ...
                                $rollupruleid = insert_record('udutu_seq_rolluprule',$rollup);
                                if (isset($rollup->conditions)) {
                                    foreach($rollup->conditions as $condition){
                                        ...
                                        $conditionid = insert_record('udutu_seq_rolluprulecond',$cond);
                                    }
                                }
                            } 
                        }
                        ...
                        if (isset($item->objectives)) {
                            foreach($item->objectives as $objective) {
                                ...
                                $objectiveid = insert_record('udutu_seq_objective',$obj);
...
//print_object($objective->mapinfos);
                                    foreach($objective->mapinfos as $objmapinfo) {
                                        ...
                                        $mapinfoid = insert_record('udutu_seq_mapinfo',$mapinfo);
                                    }
                                }
                            }
                        }
//print_object($item);
            ...
            if (!empty($olditems)) {
                foreach($olditems as $olditem) {
                   delete_records('udutu_scoes','id',$olditem->id);
                   delete_records('udutu_scoes_data','scoid',$olditem->id);
                   delete_records('udutu_scoes_track','scoid',$olditem->id);
                   delete_records('udutu_seq_objective','scoid',$olditem->id);
                   delete_records('udutu_seq_mapinfo','scoid',$olditem->id);
                   delete_records('udutu_seq_ruleconds','scoid',$olditem->id);
                   delete_records('udutu_seq_rulecond','scoid',$olditem->id);
                   delete_records('udutu_seq_rolluprule','scoid',$olditem->id);
                   delete_records('udutu_seq_rollupcond','scoid',$olditem->id);
                }
            }
            set_field('udutu','version',$scoes->version,'id',$udutuid);
        }
    } 
...
function udutu_optionals_data($item, $standarddata) {
    $result = array();
...
function udutu_is_leaf($sco) {
    if (get_record('udutu_scoes','udutu',$sco->udutu,'parent',$sco->identifier)) {
        return false;
    }
    return true;
}

function udutu_get_parent($sco) {
    if ($sco->parent != '/') {
        if ($parent = get_record('udutu_scoes','udutu',$sco->udutu,'identifier',$sco->parent)) {
            return udutu_get_sco($parent->id);
        }
    }
    return null;
}

function udutu_get_children($sco) {
    if ($parent = get_records('udutu_scoes','udutu',$sco->udutu,'parent',$sco->identifier)) {
        return $children;
    }
    return null;
}

function udutu_get_sibling($sco) {
    if ($siblings = get_records('udutu_scoes','udutu',$sco->udutu,'parent',$sco->parent)) {
