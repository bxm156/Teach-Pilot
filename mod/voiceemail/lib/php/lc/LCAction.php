...
class LCAction {
    ...
    function LCAction($session, $server, $login, $password, $path, $courseId = "")
    {
        ...
        $this->api = LCApi::getInstance($server, $login, $password,$this->prefix, $path);
    ...
    function createFirstTime()
    {
        ...
        if ($this->api->lcapi_get_users($this->getStudentUserid()) === false) 
        {
            ...
            $this->api->lcapi_create_user($this->getStudentUserid());
        } 
        
        if ($this->api->lcapi_get_users($this->getTeacherUserid()) === false) 
        {
            // create the two users
            $this->api->lcapi_create_user($this->getTeacherUserid()); 
        } 
    } 
    ...
    function createRoom($roomId, $update)
    {
        ...
        if(!$room->isArchive())
        {
          $autoOpenArchives = $this->session->request["auto_open_archive"];
          $room->setAutoOpenArchive($autoOpenArchives);
        }
    ...
    {
        return $this->api->lcapi_get_rooms($userid, $roomid, $archive);
    } 
    function getRoom($roomid)
    {
        $this->prefixUtil->trimPrefix($roomid,$this->prefix);
        return $this->api->lcapi_get_room_info($roomid);
    } 
