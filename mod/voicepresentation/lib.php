...
function voicepresentation_add_instance($voicetool) {
  ...
  $voicetool->name = $voicetool->name;  
   ...
   if (!$voicetool->id = insert_record('voicepresentation', $voicetool)) 
  {
  ...
  $resource = get_record('voicepresentation_resources',"rid",$voicetool->resource);
  if($resource->gradeid != -1){//the grade settings is enabled for this resource
     $activity = get_record('voicepresentation',"id",$resource->gradeid);
...
function voicepresentation_update_instance($voicetool) {
  ...
  /// will update an existing instance with new data.
    ...
    if (!$voicetool->id = update_record('voicepresentation', $voicetool)) 
    {
      return false;
    }
...
function voicepresentation_addCalendarEvent($activity_informations,$instanceNumber){
    ...
    $resource=get_record('voicepresentation_resources','rid',$activity_informations->rid);
    ...
    $oldEvent=get_record('event','instance',$instanceNumber,'modulename',"voicepresentation");
...
function voicepresentation_deleteCalendarEvent($instanceNumber){
  /// Basic event record for the database.
    ...
    if(!$event=get_record('event','instance',$instanceNumber,'modulename',"voicepresentation"))
    {
...
function voicepresentation_delete_resource($rid) {
    ...
    if (! $voicetool = get_record("voicepresentation_resources", "rid", $rid)) {
        wimba_add_log(WIMBA_ERROR,voicepresentation_LOGS,"Problem to delete a resource : ".$rid); 
        return false;
    }
    # Delete any dependent records here #
    if (! delete_records("voicepresentation_resources", "id", "$voicetool->id")) {
        wimba_add_log(WIMBA_ERROR,voicepresentation_LOGS,"Problem to delete a resource : ".$rid); 
        return false;
    } 
...
function voicepresentation_store_new_element($voicetool) {
  ...
  $id=insert_record("voicepresentation_resources", $voicetool);  

  return $id;
} 

function voicepresentation_update_element($voicetool) {
  $oldId = get_record('voicepresentation_resources','rid',$voicetool->rid);
 
  $voicetool->id = $oldId->id;  
  $id=update_record("voicepresentation_resources", $voicetool);  
...
function voicepresentation_get_voicetools_list($courseId) {
    $tools_list = get_records('voicepresentation_resources','course',$courseId);
    $result= array();
    $result["rid"]= array();
    $result["info"]= array();
...
function voicepresentation_get_voicetool_informations($rid) {
    ...
    $tool = get_record('voicepresentation_resources','rid',$rid);
...
function voicepresentation_get_wimbaVoice_Informations($rid) {
    ...
    if(!($board_informations = get_record('voicepresentation_resources','rid',$rid))) 
    {
        error( "Response get_board_list : query to database failed"); 
        return "error_moodleDatabase";
    }
...
function voicepresentation_get_event_calendar($id) {
  ...
  $event = get_record('event', 'instance', $id,'modulename','voicepresentation');
...
function voicepresentation_reset_userdata($data) {
   ...
   if (!empty($data->reset_content_voicepresentation_replies)) {
        ...
        foreach (array_keys($resources) as $id) 
        {
            ...
            echo grade_get_grades($resources[$id]->course, "mod", "voicepresentation", $resources[$id]->gradeid); 
...
function voicepresentation_createResourceFromResource($rid,$new_rid,$new_course,$options="0")
{
    ...
    $voicetools = get_record("voicepresentation_resources","rid",$rid);
    $voicetools->id = null;
...
function voicepresentation_delete_grade_column($rid,$course_id,$activity_id=null){
    ...
    update_record("voicepresentation_resources",$voicetools);
