...
require_once($CFG->dirroot.'/mod/voicepresentation/lib.php');
    ...
    function voicepresentation_restore_mods($mod,$restore) {
        ...
        if ($data) {
            ...
            if($userdata === true)
            {
                ...
                $resourceDbMatched= get_record("voicepresentation_resources", "fromrid", $old_rid,"course",$restore->course_id,"copyoptions",$typeCopy);//resource which match the current copy options
            ...
            if($isGradable == 'true'){//the old vb was gradable
              ...
              if(isset($actionGradebook) && $actionGradebook == "update"){                 
                //we update the name of the column (add "with user data")
                 voicepresentation_delete_grade_column($ridForGrade, $restore->course_id, $newActivityId);//delete the one automatically created by moodle
    ...
    //voicepresentation_decode_content_links_caller() function in each module
    //in the restore process
    function voicepresentation_decode_content_links ($content,$restore) {
        ...
        $searchstring='/\$@(voicepresentationINDEX)\*([0-9]+)@\$/';
        //We look for it
        ...
        if ($foundset[0]) {
            ...
            foreach($foundset[2] as $old_id) {
                ...
                $searchstring='/\$@(voicepresentationINDEX)\*('.$old_id.')@\$/';
                //If it is a link to this course, update the link to its new location
                if($rec->new_id) {
                    //Now replace it
                    $result= preg_replace($searchstring,$CFG->wwwroot.'/mod/voicepresentation/index.php?id='.$rec->new_id,$result);
                } else { 
                    //It's a foreign link so leave it as original
                    $result= preg_replace($searchstring,$restore->original_wwwroot.'/mod/voicepresentation/index.php?id='.$old_id,$result);
                }
            }
        }
        ...
        $searchstring='/\$@(voicepresentationVIEWBYID)\*([0-9]+)@\$/';
        //We look for it
        ...
        if ($foundset[0]) {
            ...
            foreach($foundset[2] as $old_id) {
                ...
                //Personalize the searchstring
                $searchstring='/\$@(voicepresentationVIEWBYID)\*('.$old_id.')@\$/';
                //If it is a link to this course, update the link to its new location
                if($rec->new_id) {
                    //Now replace it
                    $result= preg_replace($searchstring,$CFG->wwwroot.'/mod/voicepresentation/view.php?id='.$rec->new_id,$result);
                } else {
                    //It's a foreign link so leave it as original
                    $result= preg_replace($searchstring,$restore->original_wwwroot.'/mod/voicepresentation/view.php?id='.$old_id,$result);
                }
            }
        }
    ...
    function voicepresentation_decode_content_links_caller($restore) {
        global $CFG;
        $status = true;
        $content = "";
        if ($voicefeatures = get_records_sql ("SELECT c.id
                                   ...
                                   WHERE c.course = $restore->course_id")) {
                                               //Iterate over each voicepresentation->intro
            ...
            foreach ($voicefeatures as $voicefeature) {
                ...
                if ($result != $content) {
                    ...
                    $status = update_record("voicepresentation",$voicefeature);
                    if (debugging()) {
                        if (!defined('RESTORE_SILENTLY')) {
