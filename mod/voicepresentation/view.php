...
if(indexOf($_SERVER['HTTP_REFERER'],"/grader/") != -1){
  //we come from the gradebook
    ...
    if (! $voicetool = get_record("voicepresentation", "id", $cm->instance)) 
...
if ($voicetool->isfirst == 0)
{
    ...
    update_record("voicepresentation",$voicetool); 
...
//Management of the breadcrump
	$cm->modname="voicepresentation";
	$cm->name=$voicetool->name;
    $navigation = build_navigation('', $cm);    

    print_header("$course->shortname: $voicetool->name", $course->fullname,
                    $navigation,
                    "", "", true, update_module_button($cm->id, $course->id, $strvoicetool." ".get_string('activity','voicepresentation')), navmenu($course)); 
}
else
{
    ...
    $navigation[$strvoicetools] = "$CFG->wwwroot/mod/voicepresentation/index.php?id=$course->id";
...
if($resource->error==true)
{
    ...
    error(get_string("problem_vt", "voicepresentation"), "$CFG->wwwroot/course/view.php?id=$course->id");
}
