...
class WimbaUI {
    ...
    */  
    function createLcAccessPanel() {
        ...
        if ($this->currentObject == null || $this->currentObject->getUserLimit() == -1) 
        {
           $parameters["checked"] = "true";
        }
        $this->xml->addInputElement($parameters);
        ...
        if ($this->currentObject != null) 
        {
            $parameters["value"] = $this->api->getServer() .
                                    "/launcher.cgi?room=" .
                                    $this->prefix.$this->currentObject->getRoomId();
        }
        else
        {
            $parameters["value"] = $this->api->getServer() .
    ...
    */     
    function createVBFeaturesPanel() {
        ...
        if(isGradebookAvailable()){
          ...
          if(isset ($this->currentObject)){
            $activities = get_records("voicepresentation","rid",$this->currentObject->getRid());
            if(empty($activities)){
             $parameters['disabled'] = "true";
            }
          }else{
