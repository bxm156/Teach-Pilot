...
require_once($CFG->dirroot.'/mod/voicepodcaster/lib.php');
    ...
    function voicepodcaster_restore_mods($mod,$restore) {
        ...
        if ($data) {
            ...
            if($userdata === true)
            {
                ...
                $resourceDbMatched= get_record("voicepodcaster_resources", "fromrid", $old_rid,"course",$restore->course_id,"copyoptions",$typeCopy);//resource which match the current copy options
            ...
            if($isGradable == 'true'){//the old vb was gradable
              ...
              if(isset($actionGradebook) && $actionGradebook == "update"){                 
                //we update the name of the column (add "with user data")
                 voicepodcaster_delete_grade_column($ridForGrade, $restore->course_id, $newActivityId);//delete the one automatically created by moodle
    ...
    //voicepodcaster_decode_content_links_caller() function in each module
    //in the restore process
    function voicepodcaster_decode_content_links ($content,$restore) {
        ...
        $searchstring='/\$@(voicepodcasterINDEX)\*([0-9]+)@\$/';
        //We look for it
        ...
        if ($foundset[0]) {
            ...
            foreach($foundset[2] as $old_id) {
                ...
                $searchstring='/\$@(voicepodcasterINDEX)\*('.$old_id.')@\$/';
                //If it is a link to this course, update the link to its new location
                if($rec->new_id) {
                    //Now replace it
                    $result= preg_replace($searchstring,$CFG->wwwroot.'/mod/voicepodcaster/index.php?id='.$rec->new_id,$result);
                } else { 
                    //It's a foreign link so leave it as original
                    $result= preg_replace($searchstring,$restore->original_wwwroot.'/mod/voicepodcaster/index.php?id='.$old_id,$result);
                }
            }
        }
        ...
        $searchstring='/\$@(voicepodcasterVIEWBYID)\*([0-9]+)@\$/';
        //We look for it
        ...
        if ($foundset[0]) {
            ...
            foreach($foundset[2] as $old_id) {
                ...
                //Personalize the searchstring
                $searchstring='/\$@(voicepodcasterVIEWBYID)\*('.$old_id.')@\$/';
                //If it is a link to this course, update the link to its new location
                if($rec->new_id) {
                    //Now replace it
                    $result= preg_replace($searchstring,$CFG->wwwroot.'/mod/voicepodcaster/view.php?id='.$rec->new_id,$result);
                } else {
                    //It's a foreign link so leave it as original
                    $result= preg_replace($searchstring,$restore->original_wwwroot.'/mod/voicepodcaster/view.php?id='.$old_id,$result);
                }
            }
        }
    ...
    function voicepodcaster_decode_content_links_caller($restore) {
        global $CFG;
        $status = true;
        $content = "";
        if ($voicefeatures = get_records_sql ("SELECT c.id
                                   ...
                                   WHERE c.course = $restore->course_id")) {
                                               //Iterate over each voicepodcaster->intro
            ...
            foreach ($voicefeatures as $voicefeature) {
                ...
                if ($result != $content) {
                    ...
                    $status = update_record("voicepodcaster",$voicefeature);
                    if (debugging()) {
                        if (!defined('RESTORE_SILENTLY')) {
