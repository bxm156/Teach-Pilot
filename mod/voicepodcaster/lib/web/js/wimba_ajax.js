...
function createNewResource(url,product,type,name,params){
        ...
        var url = url+"?longname="+name+"&default=true&action=createDefault&product="+product+"&type="+ type +"&" + params;
        new Ajax.Request(url,
        {
            method:'post',
            mimetype:"application/xml",
            onSuccess: function(transport){     
                ...
                if(transport.responseText != "error"){ 
                    ...
                    var elSel = $('resource');
                    var lastOptElement = $('resource').options[elSel.options.length -1 ]
