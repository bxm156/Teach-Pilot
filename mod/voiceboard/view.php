...
if(indexOf($_SERVER['HTTP_REFERER'],"/grader/") != -1){
  //we come from the gradebook
    ...
    if (! $voicetool = get_record("voiceboard", "id", $cm->instance)) 
...
if ($voicetool->isfirst == 0)
{
    ...
    update_record("voiceboard",$voicetool); 
...
//Management of the breadcrump
	$cm->modname="voiceboard";
	$cm->name=$voicetool->name;
    $navigation = build_navigation('', $cm);    

    print_header("$course->shortname: $voicetool->name", $course->fullname,
                    $navigation,
                    "", "", true, update_module_button($cm->id, $course->id, $strvoicetool." ".get_string('activity','voiceboard')), navmenu($course)); 
}
else
{
    ...
    $navigation[$strvoicetools] = "$CFG->wwwroot/mod/voiceboard/index.php?id=$course->id";
...
<link rel="STYLESHEET" href="<?php p($CFG->wwwroot) ?>/mod/voiceboard/css/StyleSheet.css"" type="text/css" />
