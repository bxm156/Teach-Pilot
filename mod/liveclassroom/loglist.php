...
<?php
        ...
        if ($logsfolder = @opendir(WIMBA_DIR.'/wimbaclassroom/logs'))
        {
            ...
            while ($logname = readdir($logsfolder)) 
            {
                if (!is_dir($logname)) {
                    ...
                    echo "<a href=".$CFG->wwwroot."/mod/liveclassroom/logs.php?action=download&log=wimbaclassroom/logs/".$logname.">".$logname." - (".filesize(WIMBA_DIR."/wimbaclassroom/logs/".$logname)." b)</a><br>";
                }
            }       
            closedir($logsfolder);
        }
